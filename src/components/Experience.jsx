...
export default function Experience() {
  ...
  return (
    <section id="experience" className="py-24 relative bg-[#0B0E14] border-t border-[#1E2638]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-[#1E2638]">
            <div>
              <div className="flex items-center gap-2 text-xs font-mono text-[#3ED6C4] mb-1">
            ...
            <div className="text-xs font-mono text-[#4ADE80] bg-[#0B0E14] px-4 py-2 rounded-xl border border-[#4ADE80]/30 flex items-center gap-2 self-start md:self-auto shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {internProjects.map((project) => {
              ...
              return (
                <div
                  key={project.title}
                  className="p-5 rounded-2xl bg-[#0B0E14] border border-[#1E2638] flex flex-col justify-between space-y-4 hover:border-[#3ED6C4]/40 transition-all group"
                >
                  <div className="space-y-3">
                    <div className="flex items-center justify-between text-xs font-mono">
                      <span className="text-[#3ED6C4] font-semibold">{project.category}</span>
                      <Icon className="w-4 h-4 text-[#8B93A7]" />
                    </div>

                    <h4 className="text-base font-bold text-[#E6E9EF] group-hover:text-[#3ED6C4] transition-colors">
                      {project.title}
                    </h4>
                    ...
                    <div className="space-y-2 pt-2 border-t border-[#1E2638]/80 text-xs font-sans">
                      <div>
                        <span className="text-[#E8A33D] font-mono text-[11px] font-bold block">WHAT WAS BUILT:</span>
                        <p className="text-[#8B93A7] leading-relaxed mt-0.5">{project.whatWasBuilt}</p>
                      </div>
                      <div>
                        <span className="text-[#4ADE80] font-mono text-[11px] font-bold block">OUTCOME & METRIC:</span>
                        <p className="text-[#8B93A7] leading-relaxed mt-0.5">{project.outcome}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3 pt-3 border-t border-[#1E2638]">
                    <div className="flex flex-wrap gap-1">
                      {project.stack.map((item) => (
                        <span
                          key={item}
                          className="text-[10px] font-mono px-2 py-0.5 rounded bg-[#12161F] text-[#8B93A7] border border-[#1E2638]"
                        >
                          {item}
                        </span>
                      ))}
                    ...
                    <a
                      ...
                      className="inline-flex items-center gap-1.5 text-xs font-mono font-bold text-[#E6E9EF] hover:text-[#3ED6C4] transition-colors"
                    >
                      <svg className="w-3.5 h-3.5 text-[#3ED6C4] fill-current" viewBox="0 0 24 24">
