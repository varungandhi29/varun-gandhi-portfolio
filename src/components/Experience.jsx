...
import { Briefcase, Calendar, CheckCircle2, ChevronRight, BarChart2, Database, Map, ExternalLink } from 'lucide-react';
...
export default function Experience() {
  ...
  return (
    <section id="experience" className="py-24 relative bg-[#070b14] border-t border-slate-800/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-slate-800">
            <div>
              <div className="flex items-center gap-2 text-xs font-semibold text-cobalt-400 mb-1">
                <span className="w-2 h-2 rounded-full bg-cobalt-400"></span>
            ...
            <div className="text-xs text-slate-300 bg-slate-900 px-3.5 py-2 rounded-xl border border-slate-800 flex items-center gap-2 self-start md:self-auto font-sans font-medium">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {internProjects.map((project) => {
              ...
              return (
                <div
                  key={project.title}
                  className="p-5 rounded-xl bg-slate-900/80 border border-slate-800/90 flex flex-col justify-between space-y-4 hover:border-cobalt-500/40 transition-all group"
                >
                  <div className="space-y-3">
                    <div className="flex items-center justify-between text-xs font-semibold">
                      <span className="text-cobalt-400">{project.category}</span>
                      <Icon className="w-4 h-4 text-slate-400" />
                    </div>

                    <h4 className="text-base font-bold text-white group-hover:text-cobalt-400 transition-colors">
                      {project.title}
                    </h4>

                    <div className="inline-flex items-center gap-1.5 text-[11px] text-emerald-400 bg-emerald-950/60 px-2.5 py-0.5 rounded-md border border-emerald-500/30 font-medium">
                  ...
                  <div className="space-y-3 pt-3 border-t border-slate-800">
                    ...
                    <a
                      ...
                      className="inline-flex items-center gap-1.5 text-xs text-slate-300 hover:text-cobalt-400 transition-colors font-medium"
                    >
                      <svg className="w-3.5 h-3.5 text-cobalt-400 fill-current" viewBox="0 0 24 24">
